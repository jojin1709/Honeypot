            ...
            def s():
                while self.running:
                    try:
                        ...
                        with open(os.path.join(LOG_DIR, "ntp_queries.jsonl"), "a", encoding="utf-8") as f: f.write(json.dumps(entry)+"\n")
                        print(f"  🕐 NTP | {ts[:19]} | {client_ip:>15} | {len(data)} bytes")
                    except socket.timeout: continue
            threading.Thread(target=s, daemon=True).start()
        except Exception as e: print(f"  ❌ NTP trap failed: {e}")
    def stop(self):
        self.running = False
        if self.sock: self.sock.close()

if __name__ == "__main__":
    print("="*50); t = NTPTrap(); t.start(); print(f"  📁 Logs: {LOG_DIR}"); print("="*50)
    try:
